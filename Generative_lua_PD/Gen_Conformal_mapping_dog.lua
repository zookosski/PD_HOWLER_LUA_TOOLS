...
GUI_SetCaption("Conformal Tunnel FX v2")
...
GUI_SetSettings(h_mode, 0, "Log-Polar Tunnel")

GUI_AddControl("Line")
GUI_AddControl("TextLabel", "=== GEOMETRY ===")
local h_zoom = GUI_AddControl("Scroller", "Zoom", 50, 1, 200)
local h_twist = GUI_AddControl("Scroller", "Twist", 0, 0, 360)
local h_branches = GUI_AddControl("Scroller", "Branches x10", 60, 10, 240)
...
GUI_SetSettings(h_palette, 1, "Pastel Dream")
    ...
    for py = 0, height - 1 do
        for px = 0, width - 1 do
            ...
            elseif mode == 1 then
                ...
                local edge_glow = math.exp(-trap_min * 2.0)
                r = r * (0.4 + 0.6 * edge_glow)
                g = g * (0.4 + 0.6 * edge_glow)
                b = b * (0.4 + 0.6 * edge_glow)
            ...
            elseif mode == 4 then
                ...
                if use_canvas then
                    local cu = angle / TWO_PI
                    local cv = radius * 0.15
                    r, g, b = sample_canvas(cu % 1, cv % 1)
                else
                    r, g, b = palette_color(t, palette_id, sat)
                end
...
local needs_canvas_cache = false
